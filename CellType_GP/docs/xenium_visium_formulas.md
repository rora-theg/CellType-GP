...
对应实现：`CellType_GP/src/score_gene_program.py` 中对 `scores` 的 `mean` 与 `MinMaxScaler` 处理。
...
对应实现：`CellType_GP/src/preprocessing.py` 中对 Visium 运行 `score_gene_programs` 并抽取 `*_score_norm` 形成 `visium_score`。
...
对应实现：`CellType_GP/src/compute_truth_score.py` 中 `groupby([spot, celltype]).mean()` 对 `*_score_norm` 求均值，并在 `pivot` 后得到宽表。
