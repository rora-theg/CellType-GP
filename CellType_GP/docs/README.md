...
说明：命令默认在 `CellType_GP/src/` 目录下运行，路径前缀 `data/` 指向同级目录 `CellType_GP/data/`。
...
- 代码结构：
  - `CellType_GP/src/`：Python 包与脚本入口；`celltype_gp_models.py`（反卷积分解）、`evaluation.py`（评估）、`preprocessing.py`（打分生成）。
  - `CellType_GP/data/`：输入 `.npz`、宽表 CSV 输出、中间 AnnData/H5；请勿版本化大文件，建议用 `.gitignore` 或符号链接本地化管理。
  - `CellType_GP/notebooks/examples/`：探索性笔记本（如 `demo_run.ipynb`）；README 中需提供对应 CLI 示例（已在“快速开始”给出）。
